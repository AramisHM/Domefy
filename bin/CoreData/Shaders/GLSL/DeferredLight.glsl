...
void PS()
{
    // If rendering a directional light quad, optimize out the w divide
        ...
        vec4 bentNormalInput = texture2D(sLightBuffer, vScreenPos);
    #else
        #ifdef HWDEPTH
    ...
    vol = min(InScatter(cCameraPosPS + dir*minT, dir, cLightPosPS.xyz, t, cLightPosPS.w) * cMyst,64.);
    float fog = clamp(pow(1. - minT/cFarClipPS,0.1+cMyst),0.,1.);
//    vol *=  fog;
    ...
    #endif
        scattering = lightColor * cMediumColor*(1.-fog)+lightColor * fog;//mix(lightColor,lightColor * cMediumColor,1.-fog)  ;
        scattering *= vol;
        diff *= max(pow(1. - Z/cFarClipPS,1.5),0.);
        //albedoInput.rgb = albedoInput.rgb * cMediumColor *  (1.-fog) + albedoInput.rgb * fog;
    #ifdef SPECULAR
        float spec = GetSpecular(normal, eyeVec, lightDir, 0.7 * 255.0) * fog;
        gl_FragColor = vec4( scattering ,0.) + diff * vec4(lightColor * (albedoInput.rgb + spec * cLightColor.a * albedoInput.aaa), 0.0);
    #else
