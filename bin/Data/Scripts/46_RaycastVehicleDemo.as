...
class Fpmed : ScriptObject
{
    void DataGate(VariantMap vin, VariantMap &vout)
    {
        ...
        if (cmds[0] == "MOVE")
        {
            ...
            log.Info(str);
            vehicle.controlsInput["ACTIVE"] = true;

            vehicle.controlsInput["BACK"] =
                Abs(mvy) > triggerVal && mvy > 0.0f;
            vehicle.controlsInput["FORWARD"] =
                Abs(mvy) > triggerVal && mvy < 0.0f;
            vehicle.controlsInput["RIGHT"] =
                Abs(mvx) > triggerVal && mvx > 0.0f;
            vehicle.controlsInput["LEFT"] =
                Abs(mvx) > triggerVal && mvx < 0.0f;
        }
        if (cmds[0] == "NMOVE")
        {
            vehicle.controlsInput["BACK"] = false;
            vehicle.controlsInput["FORWARD"] = false;
            vehicle.controlsInput["RIGHT"] = false;
            vehicle.controlsInput["LEFT"] = false;
            vehicle.controlsInput["ACTIVE"] = false;
        }

        if (cmds[0] == "VIEW")
        {
            vehicle.controls.yaw += cmds[1].ToFloat() * 1.2f;
            vehicle.controls.pitch += cmds[2].ToFloat() / 2.0f; // hemisphere vieweing
        }
        //log.Info(str);
    ...
    void HandleUpdate(StringHash eventType, VariantMap &eventData)
    {
        ...
        if (ui.focusElement is null)
        {
            ...
            if (vehicle.controlsInput["ACTIVE"].GetBool() == true)
            {
                vehicle.controls.Set(
                    CTRL_FORWARD, vehicle.controlsInput["FORWARD"].GetBool());
                vehicle.controls.Set(
                    CTRL_BACK, vehicle.controlsInput["BACK"].GetBool());
                vehicle.controls.Set(
                    CTRL_LEFT, vehicle.controlsInput["LEFT"].GetBool());
                vehicle.controls.Set(
                    CTRL_RIGHT, vehicle.controlsInput["RIGHT"].GetBool());
            }
