...
class Fpmed : ScriptObject {
    void DataGate(VariantMap vin, VariantMap &vout) {
        ...
        if (cmds[0] == "MIRROR_SLIDEMOVE") {
            log.Info("movendo slide espelhado");
            log.Info(str);
            slideComp.MirrorApplyMouseMove(IntVector2(
                cmds[1].ToFloat() * 75.0f, cmds[2].ToFloat() * 75.0f));
        }
        if (cmds[0] == "SLIDEZOOM") {
            log.Info(str);
            slideComp.SetZoom(cmds[1].ToFloat() / 5.0f);
        }
        if (cmds[0] == "TOGGLE_MIRROR") {
            log.Info(str);
            slideComp.ToggleMirrorSlide();
        }
        if (cmds[0] == "MIRROR_SLIDEZOOM") {
            log.Info(str);
            slideComp.MirrorSetZoom(cmds[1].ToFloat() / 5.0f);
        }
        if (cmds[0] == "NEXTSLIDE") {
            log.Info(str);
            slideComp.NextSlide();
        ...
        if (cmds[0] == "MIRROR_CENTERSLIDE") {
            log.Info(str);
            slideComp.MirrorSetCoordinates(IntVector2(270.26f, 24.0));
            slideComp.MirrorSetZoom(8.5f);
        }
    }
