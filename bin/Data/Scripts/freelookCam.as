...
void Update(float timeStep)
	{
		...
		if (input.mouseButtonPress[MOUSEB_LEFT])
		{
			...
			plight.color = Color(0.3+Random(0.7),0.3+Random(0.7),0.3+Random(0.7)) * 1.05;
			plight.range = 300;
			
			flyer@ lighfly = cast<flyer>(lnode.CreateScriptObject(scriptFile, "flyer"));
			lighfly.Init( node.rotation * Vector3(0,0,80));
		}
		...
		if (input.keyPress['G'])
		{
			if (sdfdebug)
			{
				scene_.GetComponent("FractalWorld").SetAttribute("SDF Debug mode",false);
				sdfdebug = false;
			} else {
				scene_.GetComponent("FractalWorld").SetAttribute("SDF Debug mode",true);
				scene_.GetComponent("FractalWorld").SetAttribute("Debug plane height",node.position.y);
				sdfdebug = true;
			}
		}
...
class flyer : ScriptObject
{
	...
	void Init(Vector3 invel)
    {
		...
		//collider = node.CreateComponent("sdfCollider");
	}
	...
	void Update(float timeStep)
	{
		...
		// if (dist<1.3f && ttl<119.8)
		// {
		// 	//Vector3 n = collider.GetNorm();
		// 	n.Normalize();
			
		// 	if(0.2>n.DotProduct(vel.Normalized()))
		// 	{
				
		// 		float dt =  vel.DotProduct(n);
		// 		vel = Vector3(vel.x - 2*dt*n.x,vel.y - 2*dt*n.y,vel.z - 2*dt*n.z) * 0.7;
		// 		//vel += n * 0.05*timeStep;
				
		// 	}
		// }
		vel -= Vector3(0,6.0,0) * timeStep;
		ttl -= timeStep;
