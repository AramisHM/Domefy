...
class Fpmed : ScriptObject {
    void DataGate(VariantMap vin, VariantMap &vout) {
        ...
        if (cmds[0] == "VIEW") {
            ...
            // character.controls.pitch += cmds[2].ToFloat() / 2.0f; //
            // hemisphere vieweing
    ...
    void CreateScene() {
        ...
        // Create octree, use default volume (-1000, -1000, -1000) to (1000,
        // 1000, 1000) Also create a DebugRenderer component so that we can draw
        // debug geometry
        ...
        // occluders. Occluders will be software rasterized before rendering to
        // a low-resolution depth-only buffer to test the objects in the view
        // frustum for visibility
        Node @boxGroup = scene_.CreateChild("Boxes");
        for (uint i = 0; i < 20; ++i) {
            Node @boxNode = boxGroup.CreateChild("Box");
            float size = 1.0f + Random(10.0f);
            boxNode.position = Vector3(Random(80.0f) - 40.0f, size * 0.5f,
                                       Random(80.0f) - 40.0f);
            ...
            if (size >= 3.0f) boxObject.occluder = true;
        }

        // Create a DynamicNavigationMesh component to the scene root
        DynamicNavigationMesh @navMesh =
            scene_.CreateComponent("DynamicNavigationMesh");
        ...
        // geometry in the scene as being part of the navigation mesh. By
        // default this is recursive, but the recursion could be turned off from
        // Navigable
        scene_.CreateComponent("Navigable");
        // Add padding to the navigation mesh in Y-direction so that we can add
        // objects on top of the tallest boxes in the scene and still update the
        // mesh correctly
        navMesh.padding = Vector3(0.0f, 10.0f, 0.0f);
        // Now build the navigation geometry. This will take some time. Note
        // that the navigation mesh will prefer to use physics geometry from the
        // scene nodes, as it often is simpler, but if it can not find any (like
        // in this example) it will use renderable geometry instead
        navMesh.Build();

        // Create an off-mesh connection to each box to make it climbable (tiny
        // boxes are skipped). A connection is built from 2 nodes. Note that
        // OffMeshConnections must be added before building the navMesh, but as
        // we are adding Obstacles next, tiles will be automatically rebuilt.
        // Creating connections post-build here allows us to use
        // FindNearestPoint() to procedurally set accurate positions for the
        // connection
        CreateBoxOffMeshConnections(navMesh, boxGroup);

        // Create some mushrooms as obstacles. Note that obstacles are
        // non-walkable areas
        for (uint i = 0; i < 100; ++i)
            CreateMushroom(
                Vector3(Random(90.0f) - 45.0f, 0.0f, Random(90.0f) - 45.0f));

        // Create a CrowdManager component to the scene root (mandatory for
        // crowd agents)
        CrowdManager @crowdManager = scene_.CreateComponent("CrowdManager");
        CrowdObstacleAvoidanceParams params =
            crowdManager.GetObstacleAvoidanceParams(0);
        ...
        // Create some movable barrels. We create them as crowd agents, as for
        // moving entities it is less expensive and more convenient than using
        // obstacles
        ...
        // Create the camera. Limit far clip distance to match the fog. Note:
        // now we actually create the camera node outside the scene, because we
        // want it to be unaffected by scene load / save cameraNode = Node();
        // Camera @camera = cameraNode.CreateComponent("Camera");
        cameraNode = scene_.GetChild("CameRef");
        Camera @camera = cameraNode.GetComponent("Camera");
        camera.farClip = 300.0f;

        // Set an initial position for the camera scene node above the plane and
        // looking down
    ...
    void CreateUI() {
        // Create a Cursor UI element because we want to be able to hide and
        // show it at will. When hidden, the mouse cursor will control the
        // camera, and when visible, it will point the raycast target
    ...
    void SetupViewport() {
        // Set up a viewport to the Renderer subsystem so that the 3D scene can
        // be seen
        Viewport @viewport =
            Viewport(scene_, cameraNode.GetComponent("Camera"));
    ...
    void SubscribeToEvents() {
        ...
        // Subscribe HandlePostRenderUpdate() function for processing the
        // post-render update event, during which we request debug geometry
        SubscribeToEvent("PostRenderUpdate", "HandlePostRenderUpdate");

        // Subscribe HandleCrowdAgentFailure() function for resolving
        // invalidation issues with agents, during which we use a larger extents
        // for finding a point on the navmesh to fix the agent's position
        SubscribeToEvent("CrowdAgentFailure", "HandleCrowdAgentFailure");

        // Subscribe HandleCrowdAgentReposition() function for controlling the
        // animation
        SubscribeToEvent("CrowdAgentReposition", "HandleCrowdAgentReposition");

        // Subscribe HandleCrowdAgentFormation() function for positioning agent
        // into a formation
    ...
    void CreateMushroom(const Vector3 &pos) {
        ...
        StaticModel @mushroomObject =
            mushroomNode.CreateComponent("StaticModel");
        mushroomObject.model =
            cache.GetResource("Model", "Models/Mushroom.mdl");
        mushroomObject.material =
            cache.GetResource("Material", "Materials/Mushroom.xml");
        mushroomObject.castShadows = true;

        // Create the navigation Obstacle component and set its height & radius
    ...
    void SpawnJack(const Vector3 &pos, Node @jackGroup) {
        ...
        modelObject.material =
            cache.GetResource("Material", "Materials/Jack.xml");
        modelObject.castShadows = true;
        jackNode.CreateComponent("AnimationController");

        // Create a CrowdAgent component and set its height and realistic max
        // speed/acceleration. Use default radius
    ...
    void CreateBoxOffMeshConnections(DynamicNavigationMesh @navMesh,
                                     Node @boxGroup) {
        ...
        for (uint i = 0; i < boxes.length; ++i) {
            ...
            // Create 2 empty nodes for the start & end points of the
            // connection. Note that order matters only when using
            // one-way/unidirectional connection.
            Node @connectionStart = box.CreateChild("ConnectionStart");
            connectionStart.worldPosition = navMesh.FindNearestPoint(
                boxPos + Vector3(boxHalfSize, -boxHalfSize, 0));  // Base of box
            Node @connectionEnd = connectionStart.CreateChild("ConnectionEnd");
            connectionEnd.worldPosition = navMesh.FindNearestPoint(
                boxPos + Vector3(boxHalfSize, boxHalfSize, 0));  // Top of box

            // Create the OffMeshConnection component to one node and link the
            // other node
            OffMeshConnection @connection =
                connectionStart.CreateComponent("OffMeshConnection");
    ...
    void CreateMovingBarrels(DynamicNavigationMesh @navMesh) {
        ...
        material.textures[TU_DIFFUSE] =
            cache.GetResource("Texture2D", "Textures/TerrainDetail2.dds");
        model.castShadows = true;
        for (uint i = 0; i < 20; ++i) {
            Node @clone = barrel.Clone();
            float size = 0.5 + Random(1);
            clone.scale = Vector3(size / 1.5, size * 2.0, size / 1.5);
            clone.position = navMesh.FindNearestPoint(
                Vector3(Random(80.0) - 40.0, size * 0.5, Random(80.0) - 40.0));
    ...
    void SetPathPoint(bool spawning) {
        ...
        if (Raycast(250.0f, hitPos, hitDrawable)) {
            DynamicNavigationMesh @navMesh =
                scene_.GetComponent("DynamicNavigationMesh");
            Vector3 pathPos =
                navMesh.FindNearestPoint(hitPos, Vector3(1.0f, 1.0f, 1.0f));
            ...
            if (spawning)
                ...
                cast<CrowdManager>(scene_.GetComponent("CrowdManager"))
                    .SetCrowdTarget(pathPos, jackGroup);
        }
    }

    void AddOrRemoveObject() {
        // Raycast and check if we hit a mushroom node. If yes, remove it, if
        // no, create a new one
        Vector3 hitPos;
        Drawable @hitDrawable;
        ...
        if (Raycast(250.0f, hitPos, hitDrawable)) {
            ...
            // Note that navmesh rebuild happens when the Obstacle component is
            // removed
    ...
    bool Raycast(float maxDistance, Vector3 &hitPos, Drawable @ &hitDrawable) {
        ...
        // Check the cursor is visible and there is no UI element in front of
        // the cursor
        if (!ui.cursor.visible || ui.GetElementAt(pos, true) !is null)
            return false;

        Camera @camera = cameraNode.GetComponent("Camera");
        Ray cameraRay = camera.GetScreenRay(float(pos.x) / graphics.width,
                                            float(pos.y) / graphics.height);
        // Pick only geometry objects, not eg. zones or lights, only get the
        // first (closest) hit Note the convenience accessor to scene's Octree
        // component
        RayQueryResult result = scene_.octree.RaycastSingle(
            cameraRay, RAY_TRIANGLE, maxDistance, DRAWABLE_GEOMETRY);
    ...
    void MoveCamera(float timeStep) {
        ...
        // Override the MM_RELATIVE mouse grabbed settings, to allow interaction
        // with UI
        input.mouseGrabbed = mouseDown;

        // Right mouse button controls mouse cursor visibility: hide when
        // pressed
        ...
        // Clamp the pitch between -90 and 90 degrees Only move the camera when
        ...
        if (input.mouseButtonPress[MOUSEB_LEFT]) SetPathPoint(false);
        // Add new obstacle or remove existing obstacle/agent with middle mouse
        // button
        else if (input.mouseButtonPress[MOUSEB_MIDDLE] || input.keyPress[KEY_O])
            AddOrRemoveObject();

        // Check for loading/saving the scene from/to the file
        // Data/Scenes/CrowdNavigation.xml relative to the executable directory
        if (input.keyPress[KEY_F5]) {
            File saveFile(
                fileSystem.programDir + "Data/Scenes/CrowdNavigation.xml",
                FILE_WRITE);
    ...
    void ToggleStreaming(bool enabled) {
        ...
        if (enabled) {
            int maxTiles =
                (2 * STREAMING_DISTANCE + 1) * (2 * STREAMING_DISTANCE + 1);
    ...
    void UpdateStreaming() {
        ...
        IntVector2 beginTile = VectorMax(
            IntVector2(0, 0), jackTile - IntVector2(1, 1) * STREAMING_DISTANCE);
        IntVector2 endTile =
            VectorMin(jackTile + IntVector2(1, 1) * STREAMING_DISTANCE,
                      navMesh.numTiles - IntVector2(1, 1));

        // Remove tiles
        for (uint i = 0; i < addedTiles.length;) {
            IntVector2 tileIdx = addedTiles[i];
            if (beginTile.x <= tileIdx.x && tileIdx.x <= endTile.x &&
                beginTile.y <= tileIdx.y && tileIdx.y <= endTile.y)
                ++i;
            else {
                addedTiles.Erase(i);
    ...
    void HandleUpdate(StringHash eventType, VariantMap &eventData) {
        ...
        if (useStreaming) UpdateStreaming();
    }

    void HandlePostRenderUpdate(StringHash eventType, VariantMap &eventData) {
        if (drawDebug) {
            // Visualize navigation mesh, obstacles and off-mesh connections
            cast<DynamicNavigationMesh>(
                scene_.GetComponent("DynamicNavigationMesh"))
    ...
    void HandleCrowdAgentFailure(StringHash eventType, VariantMap &eventData) {
        ...
        // If the agent's state is invalid, likely from spawning on the side of
        // a box, find a point in a larger area
        if (state == CA_STATE_INVALID) {
            // Get a point on the navmesh using more generous extents
            Vector3 newPos =
                cast<DynamicNavigationMesh>(
                    scene_.GetComponent("DynamicNavigationMesh"))
                    .FindNearestPoint(node.position, Vector3(5.0f, 5.0f, 5.0f));
            // Set the new node position, CrowdAgent component will
            // automatically reset the state of the agent
            node.position = newPos;
        }
    }

    void HandleCrowdAgentFormation(StringHash eventType,
                                   VariantMap &eventData) {
        uint index = eventData["Index"].GetUInt();
        uint size = eventData["Size"].GetUInt();
        Vector3 position = eventData["Position"].GetVector3();

        // The first agent will always move to the exact position, all other
        // agents will select a random point nearby
        if (index > 0) {
            CrowdManager @crowdManager = GetEventSender();
            CrowdAgent @agent = eventData["CrowdAgent"].GetPtr();
            eventData["Position"] = crowdManager.GetRandomPointInCircle(
                position, agent.radius, agent.queryFilterType);
        }
    }

    void HandleCrowdAgentReposition(StringHash eventType,
                                    VariantMap &eventData) {
        ...
        AnimationController @animCtrl =
            node.GetComponent("AnimationController");
        if (animCtrl !is null) {
            float speed = velocity.length;
            if (animCtrl.IsPlaying(WALKING_ANI)) {
                float speedRatio = speed / agent.maxSpeed;
                // Face the direction of its velocity but moderate the turning
                // speed based on the speed ratio and timeStep
                node.rotation =
                    node.rotation.Slerp(Quaternion(Vector3::FORWARD, velocity),
                                        10.f * timeStep * speedRatio);
                // Throttle the animation speed based on agent speed ratio
                // (ratio = 1 is full throttle)
            ...
            if (speed < agent.radius) animCtrl.Stop(WALKING_ANI, 0.5f);
        }
    }
}