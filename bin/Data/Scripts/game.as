...
class Fpmed : ScriptObject {
	...
	//SCENE
		...
		//sdfMng = scene_.CreateComponent("sdfManager");
	}
	...
	void SetupScene()
	{
		...
		testpanelMat.cullMode = CULL_NONE;
		...
		veh = scene_.InstantiateXML(cache.GetResource("XMLFile", "Objects/barkas.xml"), Vector3(0.01,30,-50),Quaternion(0,0,0));
	...
	{	
		...
		{
			Node@ tpnode = scene_.InstantiateXML(cache.GetResource("XMLFile", "Objects/barkas.xml"), cameraNode.position + cameraNode.rotation * Vector3(0,0,3.),cameraNode.rotation * Quaternion(0.,90.,0.));;
			RigidBody@ rb = tpnode.GetComponent("RigidBody");
			rb.linearVelocity = cameraNode.rotation * Vector3(0,0,55.);
	...
	void setRndCommandParam(String param)
	{
		...
		// sdfManager@ sdfMng = scene_.GetComponent("sdfManager");
		// sdfMng.SetDefines(param);
