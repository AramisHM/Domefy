...
#include "FpmedCore.h"
#include "dv_asset.h" // images for assets

char currentFile[MaxFilePathNameSize]; // the file we are edditing
...
void run_script_cb(Fl_Widget*, void* v);
...
void run_script_cb(Fl_Widget*, void* v) {
	save_cb();
	memset(applicationScriptFileDirectory,0,MaxFilePathNameSize);
	strncpy(applicationScriptFileDirectory, filename, strlen(filename));
	flag_run_script = 1;
}
...
Fl_Menu_Item menuitems[] = {
  ...
  { "&Search", 0, 0, 0, FL_SUBMENU },
    ...
    { 0 },
  { "Run", FL_COMMAND + 'r', (Fl_Callback *)run_script_cb, 0, 0},
  { 0 }
};
