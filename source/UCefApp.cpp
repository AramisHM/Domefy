...
UCefApp::~UCefApp() { uBrowserImage_ = NULL; }

Urho3D::SharedPtr<UBrowserImage> UCefApp::GetBrowserImage() {
    return uBrowserImage_;
}
...
int UCefApp::CreateAppBrowser() {
    ...
    uCefRenderHandler_ =
        new UCefRenderHandle(CEFBUF_WIDTH, CEFBUF_HEIGHT, CEFBUF_COMPONENTS);
    uBrowserImage_->Init(uCefRenderHandler_, BROWSER_RENDER_WIDTH,
                         BROWSER_RENDER_HEIGTH);
...
void UCefApp::DestroyAppBrowser() {
    ...
    // An application should handle top-level owner window close notifications
    ...
    // 10. Application's OnBeforeClose() handler is called and the browser
    // object is destroyed.
