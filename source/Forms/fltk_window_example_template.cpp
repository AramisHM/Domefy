...
//-------------------------- Funcoes da janela
//------------------------------------------------
int index_linha_selecionada_tabela_projetores()  // return -1 se nao achou,
                                                 // senao retorna um numero
                                                 // inteiro positivo
{
    char* token;
    char linha_selecionada[512];
    char* linha_browser =
        (char*)browser_projetores->text(browser_projetores->value());
...
void callback_projetor_selecionado(Fl_Widget*, void*) {
    ...
    // dialogo que informa quando tinhamos um arquivo predefinido mas nao
    // consguimos abrir ele
    if (isProjectorFileReachable ==
        0)  // arquivo projecao definido mas nao foi possivel abrir
    {
        fl_alert(
            "Nao foi possivel abrir o arquivo .spr informado, favor selcionar "
            "um arquivo de propriedades de projetores");
    }
    if (isProjectorFileReachable <=
        0)  // arquivo indefinido ou n�o foi possivel abrir
    {
        ChooseEnvFile(0, 0);  // chose the projector properties file
        selecionaProjectorIndexForm();
    }
    if (selected_proj ==
        -1)  // arquivo definido mas indice do projetor indefinido
    {
        selecionaProjectorIndexForm();
...
void callback_servidor_selecionado(Fl_Widget*, void*) {
    ...
    if (run_scripts == 1)  // we only attempt to load scripts if enabled.
    {
        // dialogo que informa quando tinhamos um arquivo predefinido mas nao
        // consguimos abrir ele
        if (isApplicationScriptFileReachable ==
            0)  // arquivo projecao definido mas nao foi possivel abrir
        {
            fl_alert(
                "N�o foi poss�vel encontrar o Script da aplica��o informado, "
                "favor seelcionar um script para rodar a aplica��o.");
        }
        if (isApplicationScriptFileReachable <=
            0)  // arquivo indefinido ou n�o foi possivel abrir
        {
            ChooseScriptFile(0, 0);  // chose the projector properties file
            // selecionaProjectorIndexForm(); // this seems to be a bug... Why
            // would you choose a projector conf if we just choosen a script (a
            // server-only type file.)?
        }
    }
#endif
}
        ...
        default:  // PICKED FILE
            if (native.filename()) {
                strncpy(applicationScriptFileDirectory,
                        (char*)native.filename(), MaxFilePathNameSize);
            }
            break;
    }
}
...
Fl_Double_Window* desenha_janela_page_1() {
    ...
    {
        Fl_Box* o =
            new Fl_Box(15, 75, 420, 90,
                       "ANYDOME 0.40 PROTOTYPE\nTHIS SOFTWARE IS PROTECTED "
                       "WITH A NDA TERM. USE IS PERMITTED ONLY FOR TESTING.");
...
Fl_Double_Window* desenha_janela_page_2() {
    ...
    }  // Fl_Box* o
    {
        Fl_Box* o = new Fl_Box(
            15, 70, 420, 70,
            "Please fill the following atributes to configure this projector.   ATENTION: \
...
by the you while you are setting up the infrastructure of the multi projection\
 system.");
    ...
    {
        browser_projetores =
            new Fl_Browser(100, 160, 315, 80, "Environment projectors");
        ...
        for (int i = 0; i < globalEnv.getNumberOfProjectors(); i++) {
            char temp_text[50];
            sprintf(temp_text, "%d. %s", i,
                    globalEnv.getProjector(i).getName());
