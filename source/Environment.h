/*	This file is part of the FpMED (Anydome) Project Copyright (c) 2014-2016 Aramis Hornung Moraes
...
namespace fpmed {
	...
	private:
		...
		char infoName[MediumDataLen], manufacturerName[MediumDataLen], authorName[MediumDataLen];
		fpmed::Vec2<float> location; // location of the digital theater
		// General atributes for environment
		int projection_type, projection_configuration; // Type refers to either fulldome or planar.
													   // Configuration refers to the way the system
													   // is displaying, i.e. 3 monitor in line, duet
													   // fulldome with stereoscopy etc...
		fpmed::Vec2<int> resolution; // You may note by now that we only work with multiple equipaments 
