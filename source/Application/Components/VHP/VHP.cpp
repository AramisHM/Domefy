...
void VHP::CreateModel(String filePath) {
  ...
  for (int h = 0; h < axialSliceQuantity; h += 900) {
    ...
    sliceNode->SetPosition(
        Vector3(0.0f, ((axialSliceQuantity - h) * sliceAxialInterval), 0.0f));
  ...
  for (int h = 0; h < coronalSliceQuantity; h += 2) {
    ...
    m->SetTechnique(0, cache->GetResource<Technique>(
                           "Techniques/DiffAlphaTranslucent.xml"));
    Urho3D::Texture2D* t = cache->GetResource<Urho3D::Texture2D>(
        Urho3D::String(_coronalTexturesPath.c_str()) + "/" + Urho3D::String(h) +
        ".png");
    if (!t) continue;
#ifdef FPMED_LATEST_URHO3D
    t->SetFilterMode(FILTER_NEAREST_ANISOTROPIC);
  ...
  _rootNode->SetPosition(
      Vector3((float)(-modelNormalWidth / 2), 0.0f,
              (float)(-modelNormalDepth / 2)));  // this way we shift the center
                                                 // to the middle of the model
}
...
void VHP::UpdateWhatBaseToShow() {
  if (_viewingReferenceNode) {
    ...
    double yAngle = 0.0f;  // FIXME: need to be done.
    GrabbableUI* camGrab = _viewingReferenceNode->GetComponent<GrabbableUI>();
    ...
    if (_legt == false) {
      if ((camLat > 45 && camLat < 91)) {
        ...
        if ((yAngle > 45 && yAngle < 135) || (yAngle > 225 && yAngle < 315)) {
          SetSagitalBaseVisible(false);
...
void VHP::SetModelTransparency(float level) {
  ...
  for (int h = 0; h < axialSliceQuantity; ++h) {
    ...
    Variant va = Variant(Vector4(_modelColor.r_, _modelColor.g_, _modelColor.b_,
                                 _modelTransparency));
...
void VHP::UpdateAnatomicCuts() {
  ...
  if (_currentReferenceBase == axial) {
    ...
    for (int h = 0; h < axialSliceQuantity; ++h) {
      ...
      m->SetUVTransform(
          ...
          0,                                  // rotation
          Vector2(1.0f - (_currentSagitalLeftLevel + _currentSagitalRightLevel),
  ...
  if (_sagitalOngoingCut == true) {
    // left
    {
      ...
      int direction =
          (_currentSagitalLeftLevel - _targetSagitalLeftLevel > 0 ? -1 : 1);
    ...
    // right
    {
      // get transition direction
      int direction =
          (_currentSagitalRightLevel - _targetSagitalRightLevel > 0 ? -1 : 1);
  ...
  if (_currentReferenceBase == sagital) {
    ...
    for (int h = 0; h < sagitalSliceQuantity; ++h) {
      ...
      m->SetUVTransform(
          ...
          0,                                 // rotation
          Vector2(1.0f - (_currentCoronalFrontLevel + _currentCoronalBackLevel),
  ...
  if (_currentReferenceBase == coronal) {
    ...
    for (int h = 0; h < coronalSliceQuantity; ++h) {
      ...
      m->SetUVTransform(
          ...
          Vector2(1.0f - (_currentSagitalLeftLevel + _currentSagitalRightLevel),
                  1.0f - (_currentAxialUpperLevel + _currentAxialLowerLevel)));
