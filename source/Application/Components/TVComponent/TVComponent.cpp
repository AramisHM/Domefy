...
{
    SubscribeToEvent(E_SCENEPOSTUPDATE,
    ...
    if (OpenFile(name)) {
        ...
        if (theoraPacketsFound) {
            ...
            if (0 == theora_decode_init(&m_TheoraState, &m_TheoraInfo)) {
                ...
                framesPerSecond_ =
                    static_cast<float>(m_TheoraInfo.fps_numerator) /
                    static_cast<float>(m_TheoraInfo.fps_denominator);
                videoTimer_ = 0;
                isStopped_ = false;
            }
        }
...
bool TVComponent::SetOutputModel(StaticModel* sm) {
    ...
    if (sm) {
        ...
        InitTexture();
        // InitCopyBuffer();
        // ScaleModelAccordingVideoRatio(); //SRLS
    }

    return ret;
}

void TVComponent::Play() { isStopped_ = false; }

void TVComponent::Pause() { isStopped_ = true; }
...
bool TVComponent::InitTexture() {
    ...
    for (int i = 0; i < YUV_PLANE_MAX_SIZE; ++i) {
        ...
        outputTexture[i]->SetSize(texWidth, texHeight,
                                  Graphics::GetLuminanceFormat(),
                                  TEXTURE_DYNAMIC);
        outputTexture[i]->SetFilterMode(FILTER_BILINEAR);
        outputTexture[i]->SetNumLevels(1);
        outputTexture[i]->SetAddressMode(TextureCoordinate::COORD_U,
                                         TextureAddressMode::ADDRESS_MIRROR);
        outputTexture[i]->SetAddressMode(TextureCoordinate::COORD_V,
                                         TextureAddressMode::ADDRESS_MIRROR);
    }

    // assign planes textures into sepparated samplers for shader
    outputMaterial->SetTexture(TextureUnit::TU_DIFFUSE,
                               outputTexture[YUV_PLANE_Y]);
    outputMaterial->SetTexture(TextureUnit::TU_SPECULAR,
                               outputTexture[YUV_PLANE_U]);
    outputMaterial->SetTexture(TextureUnit::TU_NORMAL,
                               outputTexture[YUV_PLANE_V]);
    ...
    for (int y = 0; y < m_YUVFrame.uv_height; ++y) {
        for (int x = 0; x < m_YUVFrame.uv_width; ++x) {
            ...
            framePlanarDataU_[offsetUV] =
                m_YUVFrame.u[x + y * m_YUVFrame.uv_stride];
            framePlanarDataV_[offsetUV] =
                m_YUVFrame.v[x + y * m_YUVFrame.uv_stride];

            // 2x2 more work for Y

            const int offsetLUTile = x + y * m_YUVFrame.y_width;
            framePlanarDataY_[offsetLUTile] =
                m_YUVFrame.y[x + y * m_YUVFrame.y_stride];

            const int offsetRUTile =
                m_YUVFrame.uv_width + x + y * m_YUVFrame.y_width;
            framePlanarDataY_[offsetRUTile] =
                m_YUVFrame.y[m_YUVFrame.uv_width + x + y * m_YUVFrame.y_stride];

            const int offsetLBTile =
                x + (y + m_YUVFrame.uv_height) * m_YUVFrame.y_width;
            framePlanarDataY_[offsetLBTile] =
                m_YUVFrame
                    .y[x + ((y + m_YUVFrame.uv_height) * m_YUVFrame.y_stride)];

            const int offsetRBTile =
                (m_YUVFrame.uv_width + x) +
                (y + m_YUVFrame.uv_height) * m_YUVFrame.y_width;
            framePlanarDataY_[offsetRBTile] =
                m_YUVFrame
                    .y[(m_YUVFrame.uv_width + x) +
                       ((y + m_YUVFrame.uv_height) * m_YUVFrame.y_stride)];
        }
    }
...
void TVComponent::ScaleModelAccordingVideoRatio() {
    if (outputModel) {
        ...
        node->SetScale(Vector3(originalScale.x_, originalScale.x_ * ratioH,
                               originalScale.z_ * ratioH));
    }
}