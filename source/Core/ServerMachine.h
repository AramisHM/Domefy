...
typedef struct ServerProperties  // A struct that stores our list of servers
                                 // that the projector can try communicate to.
{
    int ComPort, SceneReplicationPort;  // The system communication port for
                                        // handshake, and the port used for
                                        // scene replication by Urho3D
...
class ServerMachine {
   ...
   public:
    ...
    int UpdateServerBeacon();  // Make server beacon listen for projectors
                               // trying to discover any server
