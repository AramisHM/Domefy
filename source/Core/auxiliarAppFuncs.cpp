 ...
 *	three-dimensional space and to calculate the angle between two points in
 *the three-dimensional space. I would appreciate  if you keep this credits.
 *	Project duration until first release -  Started (23.08.2012) - Ended
 *(01.09.2012) (10 Days) Organizing code - Started (02.09.2012) - Ended
 *(1.11.2011) Version 1.0.0.0
 */

/* You may ask yourself: why would you use this function and not the Game
Engine's quaternions? Well this is pure C and the only dependency here is the
standard math.h library. */

// If the answer above isn't ok for you, the short explanation is was in a
// hurry.
// TODO: replace this code with a proper urho3d functions
...
// The function have been modifyed to return the z axis variation from two
// points.

double triangulateTarget(double x0, double y0, double z0, double x1, double y1,
                         double z1) {
    ...
    double maximum_distance, distance,
        limit, /* How near objects are adopting the reference of maximum
                  distance [0-1] */
        ...
        hypotenuse_X_axis, hypotenuse_Y_axis, hypotenuse_Z_axis, sine_X_axis,
        cosine_X_axis, sine_Y_axis, cosine_Y_axis, sine_Z_axis, cosine_Z_axis,
        angle_X_axis, angle_Y_axis, angle_Z_axis,
        ...
        rotated_angle_X_axis, rotated_angle_Y_axis, rotated_angle_Z_axis,
        rotated_sine_X_axis, rotated_cosine_X_axis, rotated_sine_Y_axis,
        rotated_cosine_Y_axis, rotated_sine_Z_axis, rotated_cosine_Z_axis,
        ...
        result_hypotenuse_X_axis, result_hypotenuse_Y_axis,
        result_hypotenuse_Z_axis, result_sine_X_axis = 0,
        result_cosine_X_axis = 0, result_sine_Y_axis = 0,
        result_cosine_Y_axis = 0, result_sine_Z_axis = 0,
        result_cosine_Z_axis = 0, result_angle_X, result_angle_Y,
        result_angle_Z;
    ...
    printf("\nSet the maximum distance to check if the points are inside this
    limit: "); scanf("%lf",&maximum_distance); printf("\nPoint A plus extra
    ...
    */
    ...
    if (angle_X_axis < 0) angle_X_axis = (360 + angle_X_axis);
    ...
    if (angle_Y_axis < 0) angle_Y_axis = (360 + angle_Y_axis);
    ...
    if (angle_Z_axis < 0) angle_Z_axis = (360 + angle_Z_axis);
    ...
    if (result_angle_Z < 0) result_angle_Z = (360 + result_angle_Z);
    ...
    if (result_angle_X < 0) result_angle_X = (360 + result_angle_X);
    if (result_angle_Y < 0) result_angle_Y = (360 + result_angle_Y);
    ...
    // distance); printf("Those points are %f in a scale of 0 to 1 near to each
    // other\nadopting the reference of %f that is the maximum distance.\n\n",
    // limit, maximum_distance); printf("result angle X: %f\n", result_angle_X);
