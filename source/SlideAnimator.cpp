...
SlideTransitionAnimatior::SlideTransitionAnimatior(Context* context) :
...
void SlideTransitionAnimatior::SetParameters(slideElement se, float *pPitch, float *pYaw, float *pRadius, float moveSpeed, float rotationSpeed)
{
	refPitch_ = pPitch;
	refYaw_ = pYaw;
	refRadius = pRadius;
...
void SlideTransitionAnimatior::Update(float timeStep)
{
...
void SlideTransitionAnimatior::setCameraUpdateCallback(void (*f)()) {
	cameraUpdateCallback = f;
}

void SlideTransitionAnimatior::setCallbackAfterExec(void (*f)()) {
	callbackAfterExec = f;
}