...
int find_any_local_server= 0; // Should projectors ask for and connect to any server running in local network?

int run_scripts = 1; // should server run scripts?


int calibrationServerPort = defaultCalibrationServerPort; // port for server to calibrate projector view
...
void loadIniFile(char *iniFile)
{
		...
		case EXN_ELEMENT:
			{
				...
				{
					find_any_local_server = atoi(xml->getAttributeValue("value"));
				}
				...
				if (!strcmp("DefaultProjectorFile", xml->getNodeName()))
				{
					if(!strlen(projectorPropertiesFileDirectory))
					{
						...
						if(isProjectorFileReachable > 0)
						{
							...
							if(projFileData)
							{
								strncpy(projectorPropertiesFileDirectory, projFileData, strlen(projFileData));
								char *fileData2 = 0;
								fileData2 = (char*)xml->getAttributeValue("index");
				...
				if (!strcmp("ApplicationScript", xml->getNodeName()))
				{
					...
					{
						int indexVal = -1;
						if(isScriptFile && applicationScriptFileDirectory[0] == 0) // if scripts are allowd and if we have not loaded a script until now, then load this one in the .ini file
						{
							strncpy(applicationScriptFileDirectory, scriptFileName, strlen(scriptFileName));
						}
					}
				}
			}
	...
	if(strlen(serverPropertiesFileDirectory))
		strncpy(serverPropertiesFileDirectory,defaultServerFileName,strlen(defaultServerFileName));
	delete xml;
...
void defineSoftwareDirecotry(int argc, char*argv[]) {
	...
	#ifdef _WIN32
	lastOccur = strrchr(argv[0],'\\'); // THIS WILL DESTROY YOUR DAY - UNDER ANY *UNIX SYSTEM
	#else
	...
	if(argc > 1) // get the parameter file data and process it
	{
        ...
        if(!lastOccur)
            lastOccur = argv[1];
        #endif
		originalSize = strlen(argv[1]),
		ignoreSize = strlen(lastOccur);
		strncpy(executableFileExtensionDirectory, argv[1], (originalSize-ignoreSize));
		executableFileExtensionDirectory[originalSize-ignoreSize]=0;
		printf("%s DIR: %s\n", strrchr(argv[1],'.'), executableFileExtensionDirectory);

		//determin what this file will do in the system
		char *fileExtension = strrchr(argv[1],'.');

		if(!strcmp(fileExtension,serverPropertiesFileExtension)) //its a server properties file
		{
		...
		else if (!strcmp(fileExtension,".as")) // angelscript, probably the game script
		{
			strncpy(applicationScriptFileDirectory, argv[1], strlen(argv[1]));
			selected_serv = 1; // if we wan to run a script, we should be a server, tight? ;)
		}
	}
}
...
void fpmedInit(int argc, char*argv[]) {
	...
	if (argc <= 1) // show profile window choice
		...
		selecionaProjectorIndexForm();					// define qual configuração de projetor usar

	// Loop FLTK until user defines the primary application behavior/profile (server or projector).
	while (Fl::check() &&								  // atualiza a UI
		   (selected_serv < 0 ||						  // While it's not a server
			(selected_serv == 0 && selected_proj == -1))) // While is not exclusively a projector
	{
	}
}
